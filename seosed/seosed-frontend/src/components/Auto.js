...
function Auto() {
  ...
  function lisa() {
    ...
    fetch("http://localhost:8080/autod", {
      ...
      headers: {"Content-Type" : "application/json"} // Kui on valesti siis viga 415 Unsupported media type
    })                                              //see rida konventeerib JSON kujule, ja kui ei õnnestu siis jätab Stringi, mida ei võeta vastu.
    .then(res => res.json())                        //kui pole body-t siis ei ole ka headers-it vaja
    .then(json => window.location.reload())         //Reload-imine ei ole ideaalne lahendus, võiks ikka tavapäraselt töödata.
  }
    ...
    <div>
      ...
      {autod.map(auto =>
        <div key={auto.id}>
          ...
          <div>{auto.mass}</div>
          <div>{auto.aasta}</div>
          {/* ? --> kui lähen punktiga edasi järgmni asju küsima aga on võimalus, et seda pole olemas. */}
