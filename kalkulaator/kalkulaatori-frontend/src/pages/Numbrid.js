...
function Numbrid() {
    ...
    function kustuta(index) {
      ...
      function kustutaJuhuslik() {
      fetch("http://localhost:8080/kustuta-juhuslik-number")
      .then(res => res.json())
      .then(json => setNumbrid(json))
    }
    ...
    function lisa() {
      ...
      function lisaJuhuslik() {
      fetch("http://localhost:8080/lisa-juhuslik-number")
      .then(res => res.json())
      .then(json => setNumbrid(json))
    }
    ...
    function saaKogusumma() {
      ...
      function koguArv() {
      fetch("http://localhost:8080/koguarv")
      .then(res => res.json())
      .then(json => setKogusumma(json))
    }
  
       function keskmine() {
      fetch("http://localhost:8080/keskmine")
      .then(res => res.json())
      .then(json => setKogusumma(json))
    }
    ...
    function liida() {
     ...
     function lahuta() {
      fetch("http://localhost:8080/lahuta/" + nr1Ref.current.value + "/" + nr2Ref.current.value)
      .then(res => res.json())
      .then(json => setTulemus(json))
    }
    ...
    function jaga() {
     ...
     function korruta() {
      fetch("http://localhost:8080/korruta/" + nr1Ref.current.value + "/" + nr2Ref.current.value)
      .then(res => res.json())
      .then(json => setTulemus(json))
    }
  ...
  return (
    <div>
      ...
      <button onClick={lisaJuhuslik}>Lisa juhuslik number</button>
      <br/><br/>
    {numbrid.map((number, index) =>
       <div>
        ...
        <div>{kogusumma}</div>
        <button onClick={keskmine}>Keskmine väärtus</button>
        <button onClick={koguArv}>Numbrite arv</button>
        <br/><br/>
        <button onClick={() => kustutaJuhuslik()}>Kustuta juhuslik number</button>
        <br/><br/>
        <input ref={nr1Ref} type="text"/>
        <input ref={nr2Ref} type="text"/>
        <br/><br/>
        <button onClick={liida}>Liida</button>
        <button onClick={lahuta}>Lahuta</button><br/>
